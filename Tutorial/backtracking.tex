...
\item In order to find possible values for $x$, we invoke a \emph{generating function} that provides
      ...
      function is a only superset of the set of solutions, i.e.~some of those values do not solve the
      problem.  Although it is perfectly acceptable that most of the values generated do not solve
...
As a first example,  we solve is the 8 queens puzzle.  We will see that this puzzle can be solved using
...
\item The procedure \texttt{solve} has two parameters.
      ...
      \item The first parameter \texttt{l} is a list of positions of queens that
            ...
            Technically, \texttt{l} is a list of integers.
            If \texttt{l[$i$]$=k$}, then the queen in row $i$ is placed in column $k$.  For example,
            \\[0.2cm]
            \hspace*{1.3cm}
...
can be used to solve \emph{constraint satisfaction problems}.  Figure \ref{fig:generateAndTest.stlx}
...
We proceed to discuss the details of the generation of the procedure string that is later
evaluated.  
\begin{enumerate}
