...
\item First, line 5 defines a method called \texttt{sum}.  Despite the looks of it, this
      ...
      name that is used when \textsc{SetlX} eveluate the corresponding operator addition.  
      For the operator ``\texttt{+}'' performing addition, the corresponding name
      is \texttt{sum}.  Below is a table listing the names corresponding to the operators
      provided by \textsc{SetlX}.
      ...
      \begin{table}[!hbt]
        ...
        \begin{tabular}[t]{|l|l||l|l|}
          ...
          \hline
          \texttt{*}  & \texttt{product}  & \texttt{!}  & \texttt{negation} \\
          \hline
          \texttt{/}  & \texttt{quotient}    & \texttt{==}  & \texttt{equals} \\
          \hline
          \texttt{\%}  & \texttt{modulo}     & \texttt{!=}  & \texttt{notEquals} \\
          \hline
          \texttt{\symbol{92}}  & \texttt{integerDivision}    & \texttt{<}  & \texttt{lessThan}   \\
          \hline
          \texttt{**}  & \texttt{power}     & \texttt{>}  & \texttt{greaterThan}    \\
          \hline
          \texttt{<=}  & \texttt{lessOrEqual}     & \texttt{>=}  & \texttt{greaterOrEqual}    \\
          \hline
        \end{tabular}
