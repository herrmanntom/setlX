...
can use the underscore ``\texttt{\_}'' as an anonymous variable.  For example, if
...
natural number \texttt{p} as its first argument and computes the set of all factors of \texttt{p}.  Here, a number
...
The second function \texttt{primes} takes a natural number \texttt{n} as an argument and computes the
...
keyword \texttt{procedure} are assigned to variables.  As already mentioned n the previous chapter,
...
this is very handy.  The reason is that this technique permits us to write second order functions
that take procedures as inputs and modify these procedures.  Even though we will not know the number
of parameters that the procedures given as input take, we will still be able to modify them.
