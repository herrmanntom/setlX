...
algorithms that are formulated in terms of set theory have a very straightforward implementation in
\textsc{Setl}. 
...
\textsc{Setl2} \cite{snyder90b} for several years in a number of introductory computer science
courses.  He had noticed that the adoption of \textsc{Setl} had made the abstract concepts of set theory
more accessible to the students.   Nevertheless, as the original version of
\textsc{Setl} is more than forty years old, it has a number of shortcomings.  
One issue is the fact that the syntax of \textsc{Setl} and \textsc{Setl2} is quite dated and has
proven difficult to master  for students that are mainly acquainted with \texttt{C} and \textsl{Java}.
Therefore, \textsc{Setl} has been extended into the new language \textsc{SetlX}.  The main features that have
...
\item \textsc{SetlX} provides a limited way of backtracking.
...
\item \textsc{SetlX} provides a modest plotting library that can be used graph functions or to draw
      scatter plots. 
...
Furthermore, Markus Jagiella, Patrick Robinson, Arne Röcke, and Fabian Wohriska 
have added various functionalities to the \textsc{SetlX} interpreter.
\end{abstract}
\pagebreak
