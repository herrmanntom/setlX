...
\\[0.2cm]
where $A^{-1}$ denotes the inverse of the matrix $A$.  In order to carry out this calculation in
\setlx, we run the commands shown in Figure \ref{fig:solve-linear.stlx} on page
\pageref{fig:solve-linear.stlx}.  Printing the variable \texttt{s} yields the following result:
\\[0.2cm]
\hspace*{0.3cm}
\texttt{< [ 0.9999999999999998 ]  [ 1.1102230246251565E-16 ]  [ 5.551115123125783E-17 ] >}
\\[0.2cm]
This shows that the solution to the system of equations that was given initially is
\\[0.2cm]
\hspace*{1.3cm}
$x \approx 1$, \quad $y \approx 0$, \quad and \quad $z \approx 0$.  
\\[0.2cm]
Chapter \ref{chapter:linear-algebra.tex}
will discuss the data types of vectors and matrices in more detail.
