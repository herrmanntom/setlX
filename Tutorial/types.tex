...
the features that are unique to \setlx.  However, before we discuss the more elaborate data types,
...
\\[0.2cm]
and then hitting the return key \textbf{once}\footnote{
...
\texttt{0.0} at the end of the expression because if an arithmetic expression 
...
shows a program that computes $\sqrt{2}$ as the limit of the sequence $(b_n)_n$ that is defined as
\\[0.2cm]
\hspace*{1.3cm}
...
function $\texttt{print}()$ does not return a meaningful value, the return value is the \emph{undefined value}
$\Omega$.  In \setlx, this value is written as \texttt{om}.
...
\cmd{x := 1/3;}
\\[0.2cm]
binds the variable \texttt{x} to the value \texttt{1/3}.  
\vspace*{0.3cm}
...
\item ``\texttt{||}'' denotes the logical \emph{or}, (also known as \emph{disjunction}),
      so the expression
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{a || b}
      \\[0.2cm]
      is true if either \texttt{a} or \texttt{b} is true.  It is also true if both
      \texttt{a} and \texttt{b} is true.
...
\item ``\texttt{=>}'' denotes the logical \emph{implication},
      ...
      \texttt{a => b}
      \\[0.2cm]
      is true if either \texttt{a} is false or \texttt{b} is true.  Therefore,  the expression 
      \\[0.2cm]
      \hspace*{1.3cm}
      \texttt{a => b}
      \\[0.2cm]
...
outside of the existentially quantified expression.
...
to $3$, as this is the first integer in the list \texttt{[1..10]} where $n^2 > 2^n$ holds.

\section{Sets}
The most interesting data type provided by \setlx\ is  \emph{set} type.  A \emph{set} is
...
\texttt{Result: true}
\\[0.2cm]
After all, the order of elements in a set does not matter.  This behaviour can lead to
...
While neighbouring elements of a set created by the expression ``\texttt{\{ $a$..$b$ \}}''
differ by $1$, there is a variant of the range operator that allows us to
specify the size of the difference between neighbouring elements.   For example, the expression
\\[0.2cm]
\hspace*{1.3cm}
\cmd{\{1,3..10\};}
\\[0.2cm]
...
\item ``\texttt{-}'' computes the \emph{difference} of two sets.
      ...
      In mathematics, the difference of two sets $s_1$ and $s_2$ is  written
...
\\[0.2cm]
are undefined. 
Also note the use of \emph{string interpolation} in the examples given above.
\emph{Strings interpolation} will be discussed in more detail in the section on strings.

For all of the above operators there is a variant of the assignment operator that includes
the operator.  For example, we can write
\\[0.2cm]
\hspace*{1.3cm}
\texttt{s += \{x\};}
\\[0.2cm]
...
\texttt{s -= \{x\};}
\\[0.2cm]
removes the element \texttt{x} from the set \texttt{s}.
\vspace*{0.2cm}
...
We can sum the elements of a set using the prefix operator ``\texttt{+/}''.  In order to
compute the sum 
\\[0.2cm]
\hspace*{1.3cm}
...
be empty, we can always use the expression
\\[0.2cm]
\hspace*{1.3cm}
\texttt{0 +/ s}
\\[0.2cm]
...
\\[0.2cm]
When used on sets, the operator ``\texttt{-}'' computes the difference set.
The expression given above computes the set of prime numbers smaller than $100$ correctly, as a prime number
...
\texttt{x := from(s);}
\\[0.2cm]
then afterwards  the set \texttt{s} will no longer contain the element \texttt{x}.
...
as the sets contain only rational numbers, the ordering of these numbers is the usual ordering on
numbers.   If the sets contain sets or lists, these sets are themselves compared lexicographically. 
Things get more complicated if a set is heterogenous and contains both number and sets.  Programs
should not rely on the ordering implemented in these cases.
...
\texttt{rnd(5)}
\\[0.2cm]
computes a random natural number less or equal than $5$. Essentially it is equal to \texttt{rnd(\{1..5\})};
 


\section{Lists}
Besides sets, \setlx\ also supports lists.  Syntactically, the main difference is that the
braces ``\texttt{\{}'' and ``\texttt{\}}'' are substituted with the square brackets
...
\texttt{[1,4,7,2,4,7]}
\\[0.2cm]
is a typical list.  The easiest way to construct a list is via the range operator ``\texttt{..}'':
...
\\[0.2cm]
The prefix operators ``\texttt{+/}'' and ``\texttt{*/}'' work for lists also.  For example, 
\\[0.2cm]
\hspace*{1.3cm}
\texttt{+/ [1..100]}
\\[0.2cm]
...
Lists can also be used on the left hand side of an assigment.  The statement
\\[0.2cm]
\hspace*{1.3cm}
\texttt{[x,y] := [1,2];}
\\[0.2cm]
sets the variable $x$ to 1 and $y$ to 2.  This feature can be used to swap the values of variables:
the assigment
\\[0.2cm]
\hspace*{1.3cm}
\texttt{[y,x] := [x,y];}
...
\\[0.2cm]
we have that $y_1 = y_2$, then $r$ is called a \emph{map} and represents a function.
...
$
\\[0.2cm]
The program \texttt{function.stlx} shown below in figure
\ref{fig:map.stlx} shows how maps can be used
...
\noindent
The program computes the map $r$ that represents the function 
 $x \mapsto x*x$ on the set $\{n\in \mathbb{N} \mid 1 \leq n \wedge n \leq 10 \}$.  In
 line 3, the relation is evaluated at $x=3$.  This is done using square brackets.
Finally, 
$\textsl{domain}(r)$ and $\textsl{range}(r)$ are computed.
...
\pageref{fig:buggy-map.stlx} computes the answer to this question.
...
explicitely has a big memory footprint and also requires to compute all possible function
...
to compute all 
prime numbers up to a given number $n$.  The idea is to take the set $s$ of all numbers in
...
functions are first class citizens in \setlx.
...
parses this substring as an expression and substitutes the result into the string.
For example, if the variable \texttt{n} has the value $6$, the command
\\[0.2cm]
\hspace*{1.3cm}
...
Sometimes it is necessary to turn off any kind of processing when writing a string.  This
...
\\[0.2cm]
prints the string ``\texttt{\symbol{36}1+2\symbol{36}}'', where we have added the openeing
and closing quotes to delimit the string, they are not part of the string.

Literal strings are very handy when writing \emph{regular expressions}.


\section{Terms}
\setlx\ provides first order terms similar to the terms available in the language \emph{Prolog}.
...
\\[0.2cm]
is a term with functor \texttt{F} and two arguments.  Here, the functor is just a name, it is not a
function that can be evaluated.  To demonstrate the usefulness of terms, consider
implementing \emph{ordered binary trees} in \setlx. 
...
tree.  The second argument \texttt{k1} denotes the number that is to be inserted into the binary
...
\item If the binary tree \texttt{m} is empty, then the function returns the tree
      ...
      \\[0.2cm]
      This is a binary tree containing the number \texttt{k1} at its root.  In order to check
      whether \texttt{m} is indeed empty we use the functor of the term \texttt{m}.  We test in line
...
\item If the binary tree \texttt{m} is nonempty, the functor of \texttt{m} is
      ...
      functor, which is done in line 6: The first argument \texttt{k2} is the number stored at the
      root, while the arguments \texttt{l} and \texttt{r} correspond to the left and the right
      subtree of \texttt{m} respectively.      
\end{enumerate}
