...
public class QuotientAssignment extends AbstractAssignment {
     ...
     */
    public QuotientAssignment(final AssignableOperatorExpression lhs, final OperatorExpression rhs) {
        super(lhs, rhs);
    }

    @Override
    public ReturnMessage execute(final State state) throws SetlException {
        final Value assigned = lhs.evaluate(state).quotientAssign(state, rhs.evaluate(state).clone());
