...
/**
 * stat_cauchyCDF_plot(t, s, canvas):
 *                  Plots the cumulative distribution function for cauchy distributions with given parameters 't' and 's' on a given canvas.
 */
public class PD_stat_cauchyCDF_plot extends PreDefinedProcedure {
    ...
    /** Definition of the PreDefinedProcedure 'stat_cauchyCDF_plot' */
    public final static PreDefinedProcedure DEFINITION = new PD_stat_cauchyCDF_plot();

    private PD_stat_cauchyCDF_plot() {
        super();
        addParameter(CANVAS);
        addParameter(T);
        addParameter(S);
    ...
    public Value execute(State state, HashMap<ParameterDefinition, Value> args) throws SetlException {
        ...
        for (double counter = lowerBound.toJDoubleValue(state); counter < upperBound.toJDoubleValue(state); counter += interval.toJDoubleValue(state)) {
            valueList.add(new ArrayList<Double>(Arrays.asList(counter, cd.cumulativeProbability(counter))));
        }

        return ConnectJFreeChart.getInstance().addListGraph((Canvas) canvas, valueList, "Cumulative Distribution Function (mean: " + t.toString() + ", standard deviation: " + s.toString(), Defaults.createColorScheme(color, state), false);
    }
}
