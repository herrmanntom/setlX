...
public class PD_stat_paretoCDF_plot extends PreDefinedProcedure {
    ...
    /** Definition of the PreDefinedProcedure 'stat_paretoCDF_plot' */
    public final static PreDefinedProcedure DEFINITION = new PD_stat_paretoCDF_plot();

    private PD_stat_paretoCDF_plot() {
        super();
        addParameter(SCALE);
        addParameter(SHAPE);
        addParameter(CANVAS);
    ...
    public Value execute(State state, HashMap<ParameterDefinition, Value> args) throws SetlException {
        ...
        for (double counter = lowerBound.toJDoubleValue(state); counter < upperBound.toJDoubleValue(state); counter += interval.toJDoubleValue(state)) {
            valueList.add(new ArrayList<Double>(Arrays.asList(counter, pd.cumulativeProbability(counter))));
        }

        return ConnectJFreeChart.getInstance().addListGraph((Canvas) canvas, valueList, "Cumulative Distribution Function (shape: " + shape.toString() + ", scale: " + scale.toString(), Defaults.DEFAULT_COLOR_SCHEME, false);
    }
}
