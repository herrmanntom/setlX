...
\end{lstlisting}
	\item Save and close the file.
	\item Copy the modified `\command{win-setlX.cmd}'-script into the folder, where you want to work with \setlX{} programs (e.g. `\command{C:\textbackslash{}Users\textbackslash{}<user>\textbackslash{}education\textbackslash{}setlX\textbackslash{}}').
\end{enumerate}
...
The main example for such situation is a 1st generation Apple Macintosh with Intel CPU. These Machines are artificially limited by Apple to Java 1.5 only. It is recommended that you try installing an unofficial ``Soylatte'' port of Java 1.6 for these machines, which can be found at \url{http://landonf.bikemonkey.org/static/soylatte/}. You should only attempt to rebuild \setlX{} for Java 1.5 if you can not, or do not want to install this port.
